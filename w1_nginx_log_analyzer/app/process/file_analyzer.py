...
def find_last_log_file(cfg: AppConfig) -> FileDescription:
    last_log = FileDescription(None, datetime(MINYEAR, 1, 1))
    nginx_logs = [fn for fn in os.listdir(cfg.nginx_logs_dir) if
    ...
    for fn_log in nginx_logs:
        ...
        if regex is not None:
            ...
            if file_dt > last_log.date:
                last_log = FileDescription(f"{cfg.nginx_logs_dir}/{fn_log}", file_dt)
